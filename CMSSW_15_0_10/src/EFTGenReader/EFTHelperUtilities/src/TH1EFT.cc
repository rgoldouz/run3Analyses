...
TH1EFT::TH1EFT(const char *name, const char *title, Int_t nbinsx, const Double_t *xbins)
...
WCFit TH1EFT::GetBinsFit(unsigned int bin1, unsigned int bin2)
{
    WCFit summed_fit;
    for (unsigned int i = 0; i < this->hist_fits.size(); i++) {
        if(i<bin1 || i>bin2) continue;
...
Double_t TH1EFT::GetBinContentNoWC(Int_t bin)
{
    return GetBinContent(bin);
}
