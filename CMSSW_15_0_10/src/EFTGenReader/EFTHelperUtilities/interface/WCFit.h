...
public:
    ...
    WCFit(UInt_t nWCnames , std::vector<std::string> WCnames, UInt_t nfitCoefficients, Float_t fitCoefficients[], float weight){
        this->clear();
        this->extend(kSMstr);   // The SM term is always first
        for (UInt_t i=0;i<nWCnames;++i){
            this->extend(WCnames[i]);
        }
        for (UInt_t n=0;n<nfitCoefficients;++n){
            this->coeffs.at(n) = fitCoefficients[n] * weight;
        }
        this->tag = "";
    }
    ...
    void save(std::string fpath,bool append=false) {
        ...
        for (uint i = 0; i < this->size(); i++) {
            ...
            ss1 << n1 +"*"+n2 + ":";
            ss1 << std::to_string(this->coeffs.at(i)) + "\n";
